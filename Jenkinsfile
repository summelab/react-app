pipeline {
    ...
    stages {
        ...
        stage('Build Docker Image') {
            ...
            steps {
                script {
                    app = docker.build("summelab/react-app")
                    app.inside {
        ...
        stage('Push Docker Image') {
            ...
            steps {
                script {
                    docker.withRegistry('https://registry.hub.docker.com', 'summelab') {
         ...
         stage('DeployToStaging') {
            ...
            steps {
                    script {
                        sh "docker pull summelab/react-app:${env.BUILD_NUMBER}"
                        try {
                        ...
                        sh "docker run --restart always --name react-app -p 1233:80 -d summelab/react-app:${env.BUILD_NUMBER}"
                    }
            }
        }
